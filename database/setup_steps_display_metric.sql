-- Steps Display Metric Configuration
-- Setup display metric for Steps (aggregation metric already exists)
-- Created: 2025-11-05
...
INSERT INTO display_metrics (
    ...
    '["Aim for 8,000-10,000 steps per day for optimal health benefits", "Break up long periods of sitting with short walking breaks", "Take the stairs instead of the elevator when possible", "Park farther away to add extra steps to your day", "Walk during phone calls or meetings when appropriate", "Use a fitness tracker or smartphone to monitor progress"]'::jsonb
...
INSERT INTO display_metrics_aggregations (
    ...
    -- Daily view: Show daily totals
    ('DISP_STEPS', 'AGG_STEPS', 'daily', 'SUM', 1),
    -- Weekly view: Show daily averages
    ('DISP_STEPS', 'AGG_STEPS', 'weekly', 'AVG', 1),
    -- Monthly view: Show daily averages
    ('DISP_STEPS', 'AGG_STEPS', 'monthly', 'AVG', 1),
    -- 6-Month view: Show weekly averages
    ('DISP_STEPS', 'AGG_STEPS', 'sixMonth', 'AVG', 1),
    -- Yearly view: Show monthly averages
    ('DISP_STEPS', 'AGG_STEPS', 'yearly', 'AVG', 1)
