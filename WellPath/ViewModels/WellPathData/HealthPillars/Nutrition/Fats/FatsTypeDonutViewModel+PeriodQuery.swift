//
//  FatsTypeDonutViewModel+PeriodQuery.swift
//  WellPath
//
//  Extension to query fat type data for specific periods
//  Queries patient_quantity_samples directly and groups by fat_type metadata
//
...
extension FatsTypeDonutViewModel {
    func loadDataForPeriod(period: TimePeriod, date: Date) async {
        ...
        do {
            ...
            print("🥑 Querying fat types for period: \(period)")
            print("🥑   Date range: \(startDate) to \(endDate)")

            // Query patient_quantity_samples for fat_grams with fat_type metadata
            struct FatSample: Codable {
            ...
            let samples: [FatSample] = try await supabase
                .from("patient_quantity_samples")
                .select("canonical_value, metadata, aggregation_date")
                .eq("patient_id", value: userId.uuidString)
                .eq("quantity_type", value: "fat_grams")
            ...
            print("🥑 Got \(samples.count) fat samples")

            // Group by fat_type from metadata, tracking unique dates with data
            ...
            for sample in samples {
                ...
                // Get fat_types from metadata, default to "unassigned"
                var fatType = "unassigned"
                if let metadata = sample.metadata,
                   case .string(let type) = metadata["fat_types"] {
                    fatType = type
                }

                typeTotals[fatType, default: 0] += value
            }
            ...
            typeData = typeTotals
            print("🥑 Loaded \(typeData.count) fat type values")

            // Calculate total to verify percentages
            let total = typeData.values.reduce(0, +)
            print("🥑 Total fats: \(String(format: "%.1f", total))g")
            ...
            print("❌ Error loading fat type data: \(error)")
        }
