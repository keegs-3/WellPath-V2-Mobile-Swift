...
class HealthProfileViewModel: ObservableObject {
    ...
    private func loadResponsesFromDatabase(_ dbResponses: [PatientSurveyResponse]) async {
        ...
        for (questionNumber, questionResponses) in responsesByQuestion {
            ...
            var selectedOptionTexts = Set<String>()
            var rankedOptionIds: [String]? = nil
            var freeText: String? = nil

            // First, check responseOptionId (single-select stores the UUID here)
            if let responseOptionId = firstResponse.responseOptionId {
                // Store the UUID string - will be resolved to optionId when questions load
                selectedOptionIds.insert(responseOptionId.uuidString)
            }
            ...
            if let responseText = firstResponse.responseText {
                ...
                if allLookLikeIds && !parts.isEmpty {
                    // These are option IDs or UUIDs - add to selectedOptionIds
                    // Will be resolved to optionIds when we load the question
                    for part in parts {
                        selectedOptionIds.insert(part)
                    }
                } else if !selectedOptionIds.isEmpty {
                    // We have selectedOptionIds (from responseOptionId), so responseText is the display text
                    // Store it for cross-question filtering (different questions use same option texts)
                    selectedOptionTexts.insert(responseText.trimmingCharacters(in: .whitespacesAndNewlines))
                } else {
                    // No option IDs - this is free text response
        ...
        do {
            ...
            for i in 0..<loadedQuestions.count {
                ...
                if var response = responses[qNum] {
                    // Convert any UUID-based selections to optionId strings and populate texts
                    ...
                    for selectedId in response.selectedOptionIds {
                        // If it's a UUID, look up the optionId and text
                        if let uuid = UUID(uuidString: selectedId) {
                            ...
                            // Already an optionId string - look up the text
    ...
    func selectOption(_ option: SurveyResponseOption) async {
        ...
        case .singleSelect:
            ...
            response.selectedOptionTexts = [optionText]
            responses[question.questionNumber] = response
            await saveCurrentResponse()

        case .multiSelect, .multiSelectTrimmed:
            // Multi-select: toggle this option (both ID and text)
            response.selectedOptionIds.toggle(option.optionId)
            response.selectedOptionTexts.toggle(optionText)
            responses[question.questionNumber] = response
        ...
        case .rank:
            ...
            if let index = rankedIds.firstIndex(of: option.optionId) {
                ...
                response.selectedOptionTexts.remove(optionText)
            } else {
                rankedIds.append(option.optionId)
                response.selectedOptionTexts.insert(optionText)
            }
