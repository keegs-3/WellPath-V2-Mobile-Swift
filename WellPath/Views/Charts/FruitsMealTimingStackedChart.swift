//
//  FruitsMealTimingStackedChart.swift
//  WellPath
//
//  Recreated using MealTimingStackedChart as foundation
//  Dynamic stacked bar chart for meal-based fruits distribution
//

import SwiftUI
import Charts

struct FruitsMealTimingStackedChart: View {
    let color: Color
    ...
    @StateObject private var viewModel: FruitsMealTimingViewModel
    ...
    init(color: Color) {
        ...
        _viewModel = StateObject(wrappedValue: FruitsMealTimingViewModel(baseColor: color))
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            if viewModel.mealAggregations.isEmpty && !viewModel.isLoading {
                ...
                .onChange(of: selectedPeriod) { oldValue, newPeriod in
                    ...
                    print("📊 FRUITS MEAL TIMING: Set scrollPosition=\(scrollPosition) for period=\(newPeriod)")
        ...
        .task {
            ...
            print("📊 FRUITS MEAL TIMING: Initial scrollPosition=\(scrollPosition)")
        }
    }
...
@MainActor
class FruitsMealTimingViewModel: ObservableObject {
    @Published var mealAggregations: [MealAggregation] = []
    @Published var chartData: [FruitsMealStackedData] = []
    ...
    func discoverMealAggregations() async {
        do {
            ...
            let results: [AggMetric] = try await supabase
                ...
                .like("agg_id", pattern: "AGG_FRUITS_%_SERVINGS")
                .neq("agg_id", value: "AGG_FRUITS_SERVINGS")
                .execute()
                .value
            ...
            let mealOrder: [String: Int] = [
                "Breakfast Fruits": 1,
                "Morning Snack Fruits": 2,
                "Lunch Fruits": 3,
                "Afternoon Snack Fruits": 4,
                "Dinner Fruits": 5,
                "Evening Snack Fruits": 6,
                "Other Timing Fruits": 98
            ]
            ...
            mealAggregations = results.enumerated().sorted(by: {
                ...
                let isOther = agg.aggId == "AGG_FRUITS_OTHER_SERVINGS"
                ...
                if isOther {
                    ...
                    cleanName = agg.displayName.replacingOccurrences(of: " Fruits", with: "")
    ...
    private func buildChartData(for period: TimePeriod) {
        ...
        var timeline: [FruitsMealStackedData] = []
        ...
        while currentDate <= newestDate {
            ...
            timeline.append(FruitsMealStackedData(date: barDate, mealValues: mealValues))
...
struct FruitsMealStackedData: Identifiable {
    let id = UUID()
    let date: Date
...
#Preview {
    FruitsMealTimingStackedChart(color: MetricsUIConfig.getPillarColor(for: "Healthful Nutrition"))
}
