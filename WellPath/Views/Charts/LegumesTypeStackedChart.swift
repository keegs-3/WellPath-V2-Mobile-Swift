//
//  LegumesTypeStackedChart.swift
//  WellPath
//
//  Recreated using ProteinTypeStackedChart as foundation
//  Stacked bar chart for legumes type distribution
//

import SwiftUI
import Charts

struct LegumesTypeStackedChart: View {
    let color: Color
    ...
    @StateObject private var viewModel = LegumesTypeChartViewModel()
        ...
        VStack(spacing: 0) {
            if !viewModel.isLoading {
                ...
                ScrollView {
                    ...
                    VStack(spacing: 8) {
                        ForEach(legumesTypeAggIds, id: \.self) { aggId in
...
@MainActor
class LegumesTypeChartViewModel: ObservableObject {
    @Published var chartData: [LegumesTypeStackedData] = []
    ...
    func loadData(for period: TimePeriod, typeIds: [String], typeNames: [String: String], typeColors: [String: Color]) async {
        ...
        do {
            ...
            // Fetch legumes type data with date range filters
            ...
            print("❌ Error loading legumes type data: \(error)")
        }
    ...
    private func buildTimeline(
        ...
        // Generate timeline
        var timeline: [LegumesTypeStackedData] = []
        ...
        while currentDate <= newestDate {
            ...
            let typeValues = typeIds.compactMap { aggId -> LegumesTypeValue? in
                let name = typeNames[aggId] ?? aggId
                let value = values[name] ?? 0
                let typeColor = typeColors[aggId] ?? Color.gray

                return LegumesTypeValue(name: name, value: value, color: typeColor)
            }

            timeline.append(LegumesTypeStackedData(date: barDate, typeValues: typeValues))
...
struct LegumesTypeStackedData: Identifiable {
    let id = UUID()
    let date: Date
    let typeValues: [LegumesTypeValue]
}

struct LegumesTypeValue: Identifiable {
    let id = UUID()
    let name: String
    let value: Double
    let color: Color
...
#Preview {
    LegumesTypeStackedChart(color: MetricsUIConfig.getPillarColor(for: "Healthful Nutrition"))
}
