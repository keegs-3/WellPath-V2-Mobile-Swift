//
//  PillarScoreTrendChart.swift
//  WellPath
//
//  Line chart for individual Pillar Score over time with selectable points
//

import SwiftUI
import Charts

struct PillarScoreTrendChart: View {
    let pillarName: String
    @StateObject private var viewModel: PillarScoreTrendViewModel
    @State private var selectedPeriod: TimePeriod = .week
    @State private var scrollPosition: Date = Date()
    @State private var selectedDate: Date?

    init(pillarName: String) {
        self.pillarName = pillarName
        self._viewModel = StateObject(wrappedValue: PillarScoreTrendViewModel(pillarName: pillarName))
    }
                ...
                VStack(spacing: 0) {
                    ...
                    Chart(viewModel.chartData) { dataPoint in
                        ...
                        if dataPoint.score > 0 {
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [pillarColor.opacity(0.2), pillarColor.opacity(0.05)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
...
struct PillarScoreDataPoint: Identifiable {
    let id = UUID()
    let date: Date
    let score: Double
}
...
@MainActor
class PillarScoreTrendViewModel: ObservableObject {
    @Published var chartData: [PillarScoreDataPoint] = []
    @Published var isLoading = true

    private let supabase = SupabaseManager.shared.client
    private let pillarName: String
    ...
    func loadData(for period: TimePeriod) async {
        ...
        loadTask = Task {
            ...
            do {
                ...
                // Fetch pillar score HISTORY data
                let historyResponse: [PillarScoreHistory] = try await supabase
                ...
                // Fetch pillar score CURRENT data
                let currentResponse: [PillarScoreCurrent] = try await supabase
                ...
                for record in historyResponse {
                    ...
                    allRecords.append((date: utcDate, percentage: record.pillarPercentage))
                }
                ...
                for record in currentResponse {
                    ...
                    allRecords.append((date: todayStartUTC, percentage: record.pillarPercentage))
                }

                print("📊 Loaded \(historyResponse.count) historical + \(currentResponse.count) current pillar '\(pillarName)' score records")

                // Generate empty timeline
                var timeline: [PillarScoreDataPoint] = []
                ...
                while currentDate <= newestDate {
                    ...
                    timeline.append(PillarScoreDataPoint(
                        date: barDate,
                        score: 0
                    ))
                ...
                for record in allRecords {
                    ...
                    if let index = timeline.firstIndex(where: {
                        ...
                        timeline[index] = PillarScoreDataPoint(
                ...
                if let lastDate = timeline.last?.date {
                    ...
                    timeline.append(PillarScoreDataPoint(date: dummyDate, score: 0))
                }
                ...
                if !Task.isCancelled {
                    print("❌ Error loading pillar score: \(error)")
                }
            }
    ...
    private func loadOlderData(before date: Date, period: TimePeriod) async {
        ...
        do {
            ...
            print("📜 Loaded \(historyResponse.count) older pillar score records")

            var newTimeline: [PillarScoreDataPoint] = []
            var currentDate = oldestDate
            ...
            while currentDate < date {
                ...
                newTimeline.append(PillarScoreDataPoint(date: barDate, score: 0))
            ...
            for record in allRecords {
                ...
                if let index = newTimeline.firstIndex(where: {
                    ...
                    newTimeline[index] = PillarScoreDataPoint(date: newTimeline[index].date, score: record.percentage)
                }
            }
    ...
    private func loadNewerData(after date: Date, period: TimePeriod) async {
        ...
        do {
            ...
            let currentResponse: [PillarScore] = try await supabase
                .from("patient_pillar_scores_current")
                .select()
                .eq("patient_id", value: userId.uuidString)
                .eq("pillar_name", value: pillarName)
            ...
            for record in currentResponse {
                ...
                let calendar = Calendar.current
                // Use local timezone
                let todayStartUTC = calendar.startOfDay(for: Date())
                if todayStartUTC > date {
                    allRecords.append((date: todayStartUTC, percentage: record.pillarPercentage))
                }
            }
            ...
            print("📜 Loaded \(allRecords.count) newer pillar score records")
...
#Preview {
    PillarScoreTrendChart(pillarName: "Healthful Nutrition")
}
