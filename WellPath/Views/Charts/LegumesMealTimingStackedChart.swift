//
//  LegumesMealTimingStackedChart.swift
//  WellPath
//
//  Recreated using MealTimingStackedChart as foundation
//  Dynamic stacked bar chart for meal-based legumes distribution
//

import SwiftUI
import Charts

struct LegumesMealTimingStackedChart: View {
    let color: Color
    ...
    @StateObject private var viewModel: LegumesMealTimingViewModel
    ...
    init(color: Color) {
        ...
        _viewModel = StateObject(wrappedValue: LegumesMealTimingViewModel(baseColor: color))
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            if viewModel.mealAggregations.isEmpty && !viewModel.isLoading {
                ...
                .onChange(of: selectedPeriod) { oldValue, newPeriod in
                    ...
                    print("📊 LEGUMES MEAL TIMING: Set scrollPosition=\(scrollPosition) for period=\(newPeriod)")
        ...
        .task {
            ...
            print("📊 LEGUMES MEAL TIMING: Initial scrollPosition=\(scrollPosition)")
        }
    }
...
@MainActor
class LegumesMealTimingViewModel: ObservableObject {
    @Published var mealAggregations: [MealAggregation] = []
    @Published var chartData: [LegumesMealStackedData] = []
    ...
    func discoverMealAggregations() async {
        do {
            ...
            let results: [AggMetric] = try await supabase
                ...
                .like("agg_id", pattern: "AGG_LEGUMES_%_SERVINGS")
                .neq("agg_id", value: "AGG_LEGUMES_SERVINGS")
                .execute()
                .value
            ...
            let mealOrder: [String: Int] = [
                "Breakfast Legumes": 1,
                "Morning Snack Legumes": 2,
                "Lunch Legumes": 3,
                "Afternoon Snack Legumes": 4,
                "Dinner Legumes": 5,
                "Evening Snack Legumes": 6,
                "Other Timing Legumes": 98
            ]
            ...
            mealAggregations = results.enumerated().sorted(by: {
                ...
                let isOther = agg.aggId == "AGG_LEGUMES_OTHER_SERVINGS"
                ...
                if isOther {
                    ...
                    cleanName = agg.displayName.replacingOccurrences(of: " Legumes", with: "")
    ...
    private func buildChartData(for period: TimePeriod) {
        ...
        var timeline: [LegumesMealStackedData] = []
        ...
        while currentDate <= newestDate {
            ...
            timeline.append(LegumesMealStackedData(date: barDate, mealValues: mealValues))
...
struct LegumesMealStackedData: Identifiable {
    let id = UUID()
    let date: Date
...
#Preview {
    LegumesMealTimingStackedChart(color: MetricsUIConfig.getPillarColor(for: "Healthful Nutrition"))
}
