//
//  WholeGrainsMealTimingStackedChart.swift
//  WellPath
//
//  Recreated using MealTimingStackedChart as foundation
//  Dynamic stacked bar chart for meal-based whole grains distribution
//

import SwiftUI
import Charts

struct WholeGrainsMealTimingStackedChart: View {
    let color: Color
    ...
    @StateObject private var viewModel: WholeGrainsMealTimingViewModel
    ...
    init(color: Color) {
        ...
        _viewModel = StateObject(wrappedValue: WholeGrainsMealTimingViewModel(baseColor: color))
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            if viewModel.mealAggregations.isEmpty && !viewModel.isLoading {
                ...
                .onChange(of: selectedPeriod) { oldValue, newPeriod in
                    ...
                    print("📊 WHOLE GRAINS MEAL TIMING: Set scrollPosition=\(scrollPosition) for period=\(newPeriod)")
        ...
        .task {
            ...
            print("📊 WHOLE GRAINS MEAL TIMING: Initial scrollPosition=\(scrollPosition)")
        }
    }
...
@MainActor
class WholeGrainsMealTimingViewModel: ObservableObject {
    @Published var mealAggregations: [MealAggregation] = []
    @Published var chartData: [WholeGrainsMealStackedData] = []
    ...
    func discoverMealAggregations() async {
        do {
            ...
            let results: [AggMetric] = try await supabase
                ...
                .like("agg_id", pattern: "AGG_WHOLE_GRAINS_%_SERVINGS")
                .neq("agg_id", value: "AGG_WHOLE_GRAINS_SERVINGS")
                .execute()
                .value
            ...
            let mealOrder: [String: Int] = [
                "Breakfast Whole Grains": 1,
                "Morning Snack Whole Grains": 2,
                "Lunch Whole Grains": 3,
                "Afternoon Snack Whole Grains": 4,
                "Dinner Whole Grains": 5,
                "Evening Snack Whole Grains": 6,
                "Other Timing Whole Grains": 98
            ]
            ...
            mealAggregations = results.enumerated().sorted(by: {
                ...
                let isOther = agg.aggId == "AGG_WHOLE_GRAINS_OTHER_SERVINGS"
                ...
                if isOther {
                    ...
                    cleanName = agg.displayName.replacingOccurrences(of: " Whole Grains", with: "")
    ...
    private func buildChartData(for period: TimePeriod) {
        ...
        var timeline: [WholeGrainsMealStackedData] = []
        ...
        while currentDate <= newestDate {
            ...
            timeline.append(WholeGrainsMealStackedData(date: barDate, mealValues: mealValues))
...
struct WholeGrainsMealStackedData: Identifiable {
    let id = UUID()
    let date: Date
...
#Preview {
    WholeGrainsMealTimingStackedChart(color: MetricsUIConfig.getPillarColor(for: "Healthful Nutrition"))
}
