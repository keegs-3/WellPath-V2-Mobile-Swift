//
//  VegetablesTypeStackedChart.swift
//  WellPath
//
//  Recreated using ProteinTypeStackedChart as foundation
//  Stacked bar chart for vegetables type distribution
//

import SwiftUI
import Charts

struct VegetablesTypeStackedChart: View {
    let color: Color
    ...
    @StateObject private var viewModel = VegetablesTypeChartViewModel()

    // Ordered from best (top of stack) to worst (bottom of stack)
    private let vegetablesTypeAggIds = [
        ...
        "AGG_VEGETABLES_TYPE_SQUASH",
        "AGG_VEGETABLES_TYPE_ALLIUMS",
        "AGG_VEGETABLES_TYPE_OTHER"
    ]

    private let typeDisplayNames: [String: String] = [
        "AGG_VEGETABLES_TYPE_LEAFY_GREENS": "Leafy Greens",
        "AGG_VEGETABLES_TYPE_CRUCIFEROUS": "Cruciferous",
        "AGG_VEGETABLES_TYPE_ROOT_VEGETABLES": "Root Vegetables",
        "AGG_VEGETABLES_TYPE_NIGHTSHADES": "Nightshades",
        "AGG_VEGETABLES_TYPE_SQUASH": "Squash",
        "AGG_VEGETABLES_TYPE_ALLIUMS": "Alliums",
        "AGG_VEGETABLES_TYPE_OTHER": "Other"
    ]

    // Apple Health style vibrant colors
    private let typeColors: [String: Color] = [
        "AGG_VEGETABLES_TYPE_LEAFY_GREENS": Color(red: 0.2, green: 0.8, blue: 0.3),
        "AGG_VEGETABLES_TYPE_CRUCIFEROUS": Color(red: 0.3, green: 0.85, blue: 0.4),
        "AGG_VEGETABLES_TYPE_ROOT_VEGETABLES": Color(red: 0.4, green: 0.9, blue: 0.5),
        "AGG_VEGETABLES_TYPE_NIGHTSHADES": Color(red: 0.5, green: 0.95, blue: 0.6),
        "AGG_VEGETABLES_TYPE_SQUASH": Color(red: 0.6, green: 1.0, blue: 0.7),
        "AGG_VEGETABLES_TYPE_ALLIUMS": Color(red: 0.5, green: 0.75, blue: 1.0),
        "AGG_VEGETABLES_TYPE_OTHER": Color(red: 0.9, green: 0.9, blue: 0.9)
    ]
    ...
    var body: some View {
        VStack(spacing: 0) {
            if !viewModel.isLoading {
                ...
                .onChange(of: selectedPeriod) { oldValue, newPeriod in
                    ...
                    print("📊 VEGETABLES TYPE: Set scrollPosition=\(scrollPosition) for period=\(newPeriod)")

                    Task {
                        await viewModel.loadData(
                            for: newPeriod,
                            typeIds: vegetablesTypeAggIds,
                ...
                ScrollView {
                    ...
                    VStack(spacing: 8) {
                        ForEach(vegetablesTypeAggIds, id: \.self) { aggId in
        ...
        .task {
            ...
            print("📊 VEGETABLES TYPE: Initial scrollPosition=\(scrollPosition)")
        }
    }
...
@MainActor
class VegetablesTypeChartViewModel: ObservableObject {
    @Published var chartData: [VegetablesTypeStackedData] = []
    ...
    func loadData(for period: TimePeriod, typeIds: [String], typeNames: [String: String], typeColors: [String: Color]) async {
        ...
        do {
            ...
            // Fetch vegetables type data with date range filters
            ...
            print("🥬 Fetched \(results.count) vegetables type data points for range \(oldestDate) to \(newestDate)")
            ...
            print("❌ Error loading vegetables type data: \(error)")
        }
    ...
    private func buildTimeline(
        ...
        // Generate timeline
        var timeline: [VegetablesTypeStackedData] = []
        ...
        while currentDate <= newestDate {
            ...
            let typeValues = typeIds.compactMap { aggId -> VegetablesTypeValue? in
                let name = typeNames[aggId] ?? aggId
                let value = values[name] ?? 0
                let typeColor = typeColors[aggId] ?? Color.gray

                return VegetablesTypeValue(name: name, value: value, color: typeColor)
            }

            timeline.append(VegetablesTypeStackedData(date: barDate, typeValues: typeValues))
...
struct VegetablesTypeStackedData: Identifiable {
    let id = UUID()
    let date: Date
    let typeValues: [VegetablesTypeValue]
}

struct VegetablesTypeValue: Identifiable {
    let id = UUID()
    let name: String
    let value: Double
    let color: Color
...
#Preview {
    VegetablesTypeStackedChart(color: MetricsUIConfig.getPillarColor(for: "Healthful Nutrition"))
}
