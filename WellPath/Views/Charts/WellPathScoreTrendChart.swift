//
//  WellPathScoreTrendChart.swift
//  WellPath
//
//  Line chart for WellPath Score over time with selectable points
//

import SwiftUI
import Charts

struct WellPathScoreTrendChart: View {
    @StateObject private var viewModel = WellPathScoreTrendViewModel()
                ...
                VStack(spacing: 0) {
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        if let date = selectedDate,
                           let selected = viewModel.chartData.first(where: { Calendar.current.isDate($0.date, equalTo: date, toGranularity: .day) }) {
                            ...
                            HStack(alignment: .firstTextBaseline, spacing: 4) {
                                Text(String(format: "%.0f", viewModel.calculateAverage(for: selectedPeriod, scrollPosition: scrollPosition)))
                                    .font(.system(size: 48, weight: .semibold))
                                    .foregroundColor(wellPathGreen)
                            }
                        }
                    ...
                    Chart(viewModel.chartData) { dataPoint in
                        ...
                        if dataPoint.score > 0 {
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [wellPathGreen.opacity(0.2), wellPathGreen.opacity(0.05)],
                                    startPoint: .top,
                                    endPoint: .bottom
                                )
                            )
                    ...
                    .onChange(of: selectedPeriod) { _, newPeriod in
                        // Recalculate scroll position when period changes
                    ...
                    .onChange(of: scrollPosition) { oldPos, newPos in
                        ...
                        Task {
                            await viewModel.loadMoreIfNeeded(scrollPosition: newPos, period: selectedPeriod)
                        }
                    }
...
@MainActor
class WellPathScoreTrendViewModel: ObservableObject {
    @Published var chartData: [ScoreDataPoint] = []
    ...
    func loadData(for period: TimePeriod) async {
        ...
        loadTask = Task {
            ...
            do {
                ...
                let now = Date()
                let calendar = Calendar.current  // Use local calendar
                let startOfToday = calendar.startOfDay(for: now)

                // Extend timeline into future (like protein chart) to allow infinite scrolling
                ...
                switch period {
                case .day:
                    oldestDate = calendar.date(byAdding: .day, value: -7, to: startOfToday) ?? now
                case .week:
                    oldestDate = calendar.date(byAdding: .weekOfYear, value: -8, to: startOfToday) ?? now
                case .month:
                    oldestDate = calendar.date(byAdding: .month, value: -6, to: startOfToday) ?? now
                case .sixMonth:
                    oldestDate = calendar.date(byAdding: .month, value: -18, to: startOfToday) ?? now
                case .year:
                    oldestDate = calendar.date(byAdding: .year, value: -3, to: startOfToday) ?? now
                }
                ...
                let oldestDateStr = queryFormatter.string(from: oldestDate)
                let newestDateStr = queryFormatter.string(from: newestDate)  // Use newestDate, not now!

                // Date parser for Supabase PostgreSQL timestamp format: "2025-09-28T14:20:30.296905+00:00"
                let dateParser = DateFormatter()
                dateParser.dateFormat = "yyyy-MM-dd'T'HH:mm:ss.SSSSSSZZZZZ"
                dateParser.timeZone = TimeZone(identifier: "UTC")
                dateParser.locale = Locale(identifier: "en_US_POSIX")

                // Fetch WellPath score HISTORY data
                let historyResponse: [WellPathScoreHistory] = try await supabase
                ...
                // Fetch WellPath score CURRENT data
                let currentResponse: [WellPathScoreOverall] = try await supabase
                ...
                for record in historyResponse {
                    ...
                    allRecords.append((date: utcDate, percentage: record.overallPercentage))
                }

                // Add current record - place at noon today
                let todayAtNoon: Date = {
                ...
                debugFormatter.timeZone = calendar.timeZone
                print("⏰ NOW is: \(debugFormatter.string(from: now))")
                print("⏰ Today at noon is: \(debugFormatter.string(from: todayAtNoon))")

                for record in currentResponse {
                    // Current score shows at noon today
                    allRecords.append((date: todayAtNoon, percentage: record.overallPercentage))
                    print("➕ Adding current record at \(debugFormatter.string(from: todayAtNoon)) with score: \(record.overallPercentage)")
                }

                print("📊 Loaded \(historyResponse.count) historical + \(currentResponse.count) current WellPath score records")

                let dateFormatter = DateFormatter()
                dateFormatter.dateFormat = "MM/dd HH:mm"
                dateFormatter.timeZone = calendar.timeZone
                print("📅 Record dates (local): \(allRecords.map { dateFormatter.string(from: $0.date) })")
                print("📅 Timeline range: \(dateFormatter.string(from: oldestDate)) to \(dateFormatter.string(from: newestDate))")

                // Generate empty timeline
                var timeline: [ScoreDataPoint] = []
                ...
                while currentDate <= newestDate {
                    ...
                    timeline.append(ScoreDataPoint(
                        date: barDate,
                        score: 0
                    ))
                ...
                print("📊 Matched \(timeline.filter { $0.score > 0 }.count) out of \(allRecords.count) records to timeline")
                print("📊 Timeline has \(timeline.count) total slots")
                ...
                if let lastDate = timeline.last?.date {
                    ...
                    timeline.append(ScoreDataPoint(date: dummyDate, score: 0))
                }
    ...
    func loadMoreIfNeeded(scrollPosition: Date, period: TimePeriod) async {
        guard !chartData.isEmpty, !isLoading else { return }
    ...
    private func loadOlderData(before date: Date, period: TimePeriod) async {
        ...
        do {
            ...
            // Load data for one period range before earliest
            let rangeToLoad: Int
            switch period {
            case .day: rangeToLoad = 7
            case .week: rangeToLoad = 8
            case .month: rangeToLoad = 30
            case .sixMonth: rangeToLoad = 26
            case .year: rangeToLoad = 52
            }

            let oldestDate = calendar.date(byAdding: period.calendarComponent, value: -rangeToLoad, to: date) ?? date

            let queryFormatter = ISO8601DateFormatter()
            let oldestDateStr = queryFormatter.string(from: oldestDate)
            let newestDateStr = queryFormatter.string(from: date)
            ...
            let historyResponse: [WellPathScoreHistory] = try await supabase
                ...
                .eq("patient_id", value: userId.uuidString)
                .gte("calculated_at", value: oldestDateStr)
                .lt("calculated_at", value: newestDateStr)
                .order("calculated_at", ascending: true)
                .execute()
                .value

            guard !historyResponse.isEmpty else { return }
            ...
            print("📜 Loaded \(historyResponse.count) older WellPath score records")

            // Generate timeline for older data
            var newTimeline: [ScoreDataPoint] = []
            var currentDate = oldestDate
            ...
            while currentDate < date {
                ...
                newTimeline.append(ScoreDataPoint(date: barDate, score: 0))
            ...
            let granularity: Calendar.Component
            switch period {
            case .day: granularity = .hour
            case .week, .month: granularity = .day
            case .sixMonth: granularity = .weekOfYear
            case .year: granularity = .month
            }

            // Overlay data on timeline
            for record in allRecords {
                let normalizedRecordDate: Date
                switch granularity {
                case .hour: normalizedRecordDate = record.date
                case .day: normalizedRecordDate = calendar.startOfDay(for: record.date)
                case .weekOfYear: normalizedRecordDate = calendar.date(from: calendar.dateComponents([.yearForWeekOfYear, .weekOfYear], from: record.date)) ?? record.date
                case .month: normalizedRecordDate = calendar.date(from: calendar.dateComponents([.year, .month], from: record.date)) ?? record.date
                default: normalizedRecordDate = record.date
                }

                if let index = newTimeline.firstIndex(where: {
                    let normalizedTimelineDate: Date
                    switch granularity {
                    case .hour: normalizedTimelineDate = $0.date
                    case .day: normalizedTimelineDate = calendar.startOfDay(for: $0.date)
                    case .weekOfYear: normalizedTimelineDate = calendar.date(from: calendar.dateComponents([.yearForWeekOfYear, .weekOfYear], from: $0.date)) ?? $0.date
                    case .month: normalizedTimelineDate = calendar.date(from: calendar.dateComponents([.year, .month], from: $0.date)) ?? $0.date
                    default: normalizedTimelineDate = $0.date
                    }
                    return calendar.isDate(normalizedTimelineDate, equalTo: normalizedRecordDate, toGranularity: granularity)
                }) {
                    newTimeline[index] = ScoreDataPoint(date: newTimeline[index].date, score: record.percentage)
                }
            }

            // Prepend to existing data
            chartData = newTimeline + chartData

        } catch {
            print("❌ Error loading older data: \(error)")
        }
    }
    ...
    private func loadNewerData(after date: Date, period: TimePeriod) async {
        ...
        do {
            ...
            // Extend timeline into future by a large amount (like protein chart)
            // This allows infinite scrolling forward
            let rangeToLoad: Int
            switch period {
            case .day: rangeToLoad = 7
            case .week: rangeToLoad = 8
            case .month: rangeToLoad = 6  // 6 months
            case .sixMonth: rangeToLoad = 4  // 4 six-month periods
            case .year: rangeToLoad = 2  // 2 years
            }

            let newestDate = calendar.date(byAdding: period.calendarComponent, value: rangeToLoad, to: date) ?? date

            let queryFormatter = ISO8601DateFormatter()
            let oldestDateStr = queryFormatter.string(from: date)
            let newestDateStr = queryFormatter.string(from: newestDate)
            ...
            let historyResponse: [WellPathScoreHistory] = try await supabase
                ...
                .select()
                .eq("patient_id", value: userId.uuidString)
                .gt("calculated_at", value: oldestDateStr)
                .lte("calculated_at", value: newestDateStr)
                .order("calculated_at", ascending: true)
                .execute()
                .value
            ...
            debugFormatter.dateFormat = "yyyy-MM-dd HH:mm:ss Z"
            debugFormatter.timeZone = calendar.timeZone
            print("⏰ [loadNewerData] NOW is: \(debugFormatter.string(from: now))")
            print("⏰ [loadNewerData] Today at noon is: \(debugFormatter.string(from: todayAtNoon))")
            print("⏰ [loadNewerData] Comparing with date: \(debugFormatter.string(from: date))")

            for record in currentResponse {
                // Current score shows at noon today
                if todayAtNoon > date {
                    allRecords.append((date: todayAtNoon, percentage: record.overallPercentage))
                    print("➕ [loadNewerData] Adding current record at \(debugFormatter.string(from: todayAtNoon)) with score: \(record.overallPercentage)")
                } else {
                    print("⏭️ [loadNewerData] Skipping current record (todayAtNoon \(debugFormatter.string(from: todayAtNoon)) is NOT > date \(debugFormatter.string(from: date)))")
                }
            }

            guard !allRecords.isEmpty else { return }

            print("📜 Loaded \(allRecords.count) newer WellPath score records")

            // Generate timeline for newer data
            var newTimeline: [ScoreDataPoint] = []
            var currentDate = calendar.date(byAdding: period.calendarComponent, value: 1, to: date) ?? date
            ...
            chartData = chartData + newTimeline

        } catch {
            print("❌ Error loading newer data: \(error)")
        }
    }

    // Calculate average for VISIBLE WINDOW only (matches ParentMetricBarChart pattern)
    func calculateAverage(for period: TimePeriod, scrollPosition: Date) -> Double {
