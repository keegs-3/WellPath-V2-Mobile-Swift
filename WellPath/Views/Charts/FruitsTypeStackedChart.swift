//
//  FruitsTypeStackedChart.swift
//  WellPath
//
//  Recreated using ProteinTypeStackedChart as foundation
//  Stacked bar chart for fruits type distribution
//

import SwiftUI
import Charts

struct FruitsTypeStackedChart: View {
    let color: Color
    ...
    @StateObject private var viewModel = FruitsTypeChartViewModel()

    // Ordered from best (top of stack) to worst (bottom of stack)
    private let fruitsTypeAggIds = [
        ...
        "AGG_FRUITS_TYPE_MELONS",
        "AGG_FRUITS_TYPE_DRIED_FRUITS",
        "AGG_FRUITS_TYPE_OTHER"
    ]

    private let typeDisplayNames: [String: String] = [
        "AGG_FRUITS_TYPE_BERRIES": "Berries",
        "AGG_FRUITS_TYPE_CITRUS": "Citrus",
        "AGG_FRUITS_TYPE_STONE_FRUITS": "Stone Fruits",
        "AGG_FRUITS_TYPE_TROPICAL": "Tropical",
        "AGG_FRUITS_TYPE_APPLES_PEARS": "Apples & Pears",
        "AGG_FRUITS_TYPE_MELONS": "Melons",
        "AGG_FRUITS_TYPE_DRIED_FRUITS": "Dried Fruits",
        "AGG_FRUITS_TYPE_OTHER": "Other"
    ]

    // Apple Health style vibrant colors
    private let typeColors: [String: Color] = [
        "AGG_FRUITS_TYPE_BERRIES": Color(red: 0.8, green: 0.2, blue: 0.4),
        "AGG_FRUITS_TYPE_CITRUS": Color(red: 1.0, green: 0.6, blue: 0.2),
        "AGG_FRUITS_TYPE_STONE_FRUITS": Color(red: 1.0, green: 0.4, blue: 0.5),
        "AGG_FRUITS_TYPE_TROPICAL": Color(red: 1.0, green: 0.8, blue: 0.2),
        "AGG_FRUITS_TYPE_APPLES_PEARS": Color(red: 0.5, green: 0.8, blue: 0.3),
        "AGG_FRUITS_TYPE_MELONS": Color(red: 0.3, green: 0.9, blue: 0.5),
        "AGG_FRUITS_TYPE_DRIED_FRUITS": Color(red: 0.6, green: 0.4, blue: 0.2),
        "AGG_FRUITS_TYPE_OTHER": Color(red: 0.9, green: 0.9, blue: 0.9)
    ]
    ...
    var body: some View {
        VStack(spacing: 0) {
            if !viewModel.isLoading {
                ...
                .onChange(of: selectedPeriod) { oldValue, newPeriod in
                    ...
                    print("📊 FRUITS TYPE: Set scrollPosition=\(scrollPosition) for period=\(newPeriod)")

                    Task {
                        await viewModel.loadData(
                            for: newPeriod,
                            typeIds: fruitsTypeAggIds,
                ...
                ScrollView {
                    ...
                    VStack(spacing: 8) {
                        ForEach(fruitsTypeAggIds, id: \.self) { aggId in
        ...
        .task {
            ...
            print("📊 FRUITS TYPE: Initial scrollPosition=\(scrollPosition)")
        }
    }
...
@MainActor
class FruitsTypeChartViewModel: ObservableObject {
    @Published var chartData: [FruitsTypeStackedData] = []
    ...
    func loadData(for period: TimePeriod, typeIds: [String], typeNames: [String: String], typeColors: [String: Color]) async {
        ...
        do {
            ...
            // Fetch fruits type data with date range filters
            ...
            print("🍎 Fetched \(results.count) fruits type data points for range \(oldestDate) to \(newestDate)")
            ...
            print("❌ Error loading fruits type data: \(error)")
        }
    ...
    private func buildTimeline(
        ...
        // Generate timeline
        var timeline: [FruitsTypeStackedData] = []
        ...
        while currentDate <= newestDate {
            ...
            let typeValues = typeIds.compactMap { aggId -> FruitsTypeValue? in
                let name = typeNames[aggId] ?? aggId
                let value = values[name] ?? 0
                let typeColor = typeColors[aggId] ?? Color.gray

                return FruitsTypeValue(name: name, value: value, color: typeColor)
            }

            timeline.append(FruitsTypeStackedData(date: barDate, typeValues: typeValues))
...
struct FruitsTypeStackedData: Identifiable {
    let id = UUID()
    let date: Date
    let typeValues: [FruitsTypeValue]
}

struct FruitsTypeValue: Identifiable {
    let id = UUID()
    let name: String
    let value: Double
    let color: Color
...
#Preview {
    FruitsTypeStackedChart(color: MetricsUIConfig.getPillarColor(for: "Healthful Nutrition"))
}
