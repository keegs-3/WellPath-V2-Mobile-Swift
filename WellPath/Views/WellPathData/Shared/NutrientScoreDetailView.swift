//
//  NutrientScoreDetailView.swift
//  WellPath
//
//  Generic detail view for nutrient scores (vegetables, fruits, legumes, whole grains).
//  Shows Today/History/Baseline tabs.
//

import SwiftUI

struct NutrientScoreDetailView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle(config.scoreCardConfig.title)
    ...
    private var todayTabContent: some View {
        VStack(spacing: 20) {
            if viewModel.dailyScore != nil {
                ScoreRingPill(
                    score: Int(viewModel.dailyScore ?? 0),
                    iconName: config.scoreCardConfig.iconName,
                    label: "Today",
                    size: 90
                )
                .padding(.top, 8)

                Text(scoreLabel(for: Int(viewModel.dailyScore ?? 0)))
                    .font(.headline)
                    .foregroundColor(scoreColor(for: Int(viewModel.dailyScore ?? 0)))
    ...
    private var noDataTodayView: some View {
        VStack(spacing: 16) {
            Image(systemName: "calendar.badge.exclamationmark")
                .font(.system(size: 48))
                .foregroundColor(.secondary)
            Text("No \(config.title.lowercased())s logged today")
                .font(.headline)
            Text("Log your meals to see today's \(config.title.lowercased()) breakdown")
                .font(.subheadline)
                .foregroundColor(.secondary)
    ...
    private func loadScoreHistory() async {
        guard let scoreType = viewModel.dailyScoreQuantityType else {
        ...
        do {
            ...
            let results: [ScoreResult] = try await client
                ...
                .eq("quantity_type", value: scoreType)
...
#Preview {
    NutrientScoreDetailView(
        config: .legumes,
        viewModel: LegumesScoreViewModel(),
        color: .brown
    )
}
