//
//  FatsScoreDetailView.swift
//  WellPath
//
//  Detail view showing fats score with Today/History/Baseline tabs.
//  Follows ProteinScoreDetailView pattern.
//

import SwiftUI

struct FatsScoreDetailView: View {
    @ObservedObject var viewModel: FatsScoreViewModel
    let color: Color
    ...
    private var noDataTodayView: some View {
        VStack(spacing: 16) {
            Image(systemName: "calendar.badge.exclamationmark")
                .font(.system(size: 48))
                .foregroundColor(.secondary)
            Text("No fats logged today")
                .font(.headline)
            Text("Log your meals to see today's fat breakdown")
                .font(.subheadline)
                .foregroundColor(.secondary)
    ...
    private func loadScoreHistory() async {
        do {
            ...
            let results: [ScoreResult] = try await client
                ...
                .eq("quantity_type", value: "fat_score")
