...
//  WellPath
//
//  Full view for Fat Type metric (DISP_FATS_TYPE).
//  Shows fat quality score and tier breakdown with targets.
//  Focuses on "how healthy were my fat choices?"
//

import SwiftUI
import Charts

struct FatsTypeView: View {
    let color: Color
    ...
    private let metricId = "DISP_FATS_TYPE"
    private let metricName = "Fat Type"
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 0) {
                if viewModel.isLoading {
                    ...
                    // Content container with grouped background
                    VStack(spacing: 0) {
                        ...
                        VStack(spacing: 0) {
                            ...
                            HStack(alignment: .center, spacing: 12) {
                                ...
                                CompactScoreRing(score: viewModel.calculateTypeScore(), hasData: viewModel.totalFats > 0)
                            }
                            ...
                            // Donut chart with gradient colors per fat type
                            if viewModel.totalFats > 0, let tierConfig = viewModel.tierConfig {
                                Chart {
                                    ForEach(getSortedFatTypes(), id: \.self) { typeId in
        ...
        .task {
            await viewModel.loadFatTypeDisplayNames()
    ...
    private func renderTargetPercentages(_ text: String) -> some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            // Target rows
            targetPercentageRow(target: ">70%", tier: "Tier 1", sources: "olive oil, avocado, nuts", color: MetricsUIConfig.tierGood)
            targetPercentageRow(target: "~25%", tier: "Tier 2", sources: "vegetable oils, peanut butter", color: MetricsUIConfig.tierMedium)
            targetPercentageRow(target: "<5%", tier: "Tier 3", sources: "butter, lard, coconut oil", color: MetricsUIConfig.tierPoor)
    ...
    private func formatTotalFats() -> String {
        let total = viewModel.totalFats
        if total == 0 {
            return "—"
        } else if total >= 100 {
    ...
    private func calculateTierGrams(tier: FatTierConfig.Tier) -> Double {
        tier.fatTypes.reduce(0.0) { sum, typeId in
    ...
    private func calculateOtherGrams() -> Double {
        ...
        let assignedTypeIds = Set(tierConfig.tiers.flatMap { $0.fatTypes })
    ...
    /// Returns all fat types sorted by tier (1→2→3→Other), then by quantity within each tier
    private func getSortedFatTypes() -> [String] {
    ...
    /// Check if a fat type belongs to a specific tier
    private func typeInTier(_ typeId: String, tierId: String?) -> Bool {
        ...
        if let tier = tierConfig.tiers.first(where: { $0.tierId == tierId }) {
            return tier.fatTypes.contains(typeId)
        }
        return false
    }

    /// Get gradient color for a fat type based on its tier and position within that tier
    private func getGradientColor(for typeId: String) -> Color {
        ...
        for tier in tierConfig.tiers {
            if tier.fatTypes.contains(typeId) {
...
struct FatTierCard: View {
    let tier: FatTierConfig.Tier
    let tierGrams: Double
    let totalFats: Double
    let typeData: [String: Double]
    let viewModel: FatsTypeDonutViewModel
    ...
    private var tierPercentage: Double {
        guard totalFats > 0 else { return 0 }
        return (tierGrams / totalFats) * 100
    }
    ...
    private var typesWithData: [String] {
        tier.fatTypes.filter { typeId in
            (typeData[typeId] ?? 0) > 0
        }
    }
    ...
    private func calculateMarkerPosition() -> CGFloat {
        ...
        let cardWidth = screenWidth - 64
        return cardWidth * (targetPercentage / 100)
    }

    /// Get gradient color for a fat type within this tier
    ...
    private func individualTypeRow(_ typeId: String) -> some View {
        ...
        let percentage = totalFats > 0 ? (grams / totalFats) * 100 : 0
...
// MARK: - Fat Other Tier Card

struct FatOtherTierCard: View {
    ...
    private var otherPercentage: Double {
        guard totalFats > 0 else { return 0 }
        return (otherGrams / totalFats) * 100
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if isExpanded {
                VStack(spacing: 12) {
                    Text("Unclassified or mixed fat sources.")
                        .font(.caption)
                        .foregroundColor(.secondary)
                        .frame(maxWidth: .infinity, alignment: .leading)
...
#Preview {
    NavigationStack {
        FatsTypeView(color: MetricsUIConfig.getPillarColor(for: "Healthful Nutrition"))
    }
}
