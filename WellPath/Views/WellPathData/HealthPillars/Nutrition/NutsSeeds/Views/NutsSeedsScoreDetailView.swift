//
//  NutsSeedsScoreDetailView.swift
//  WellPath
//
//  Detail view showing nuts & seeds score with Today/History/Baseline tabs.
//

import SwiftUI

struct NutsSeedsScoreDetailView: View {
    @ObservedObject var viewModel: NutsSeedsScoreViewModel
    let color: Color
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("Nuts & Seeds Score")
    ...
    private var noDataTodayView: some View {
        VStack(spacing: 16) {
            Image(systemName: "calendar.badge.exclamationmark")
                .font(.system(size: 48))
                .foregroundColor(.secondary)
            Text("No nuts or seeds logged today")
                .font(.headline)
            Text("Log your intake to see today's score")
                .font(.subheadline)
                .foregroundColor(.secondary)
    ...
    private func loadScoreHistory() async {
        do {
            ...
            let results: [ScoreResult] = try await client
                ...
                .eq("quantity_type", value: "nuts_seeds_score")
...
#Preview {
    NutsSeedsScoreDetailView(
        viewModel: NutsSeedsScoreViewModel(),
        color: .brown
    )
}
