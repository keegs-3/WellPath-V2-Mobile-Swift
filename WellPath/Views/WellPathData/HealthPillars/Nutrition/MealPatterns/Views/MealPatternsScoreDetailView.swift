//
//  MealPatternsScoreDetailView.swift
//  WellPath
//
//  Detail view showing meal patterns score with Today/History/Baseline tabs.
//

import SwiftUI

struct MealPatternsScoreDetailView: View {
    @ObservedObject var viewModel: MealPatternsScoreViewModel
    let color: Color
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("Meal Patterns Score")
    ...
    private var noDataTodayView: some View {
        VStack(spacing: 16) {
            ...
            Text("No meals logged today")
                .font(.headline)
            Text("Log your meals to see today's meal patterns score")
                .font(.subheadline)
                .foregroundColor(.secondary)
    ...
    private func loadScoreHistory() async {
        do {
            ...
            let results: [ScoreResult] = try await client
                ...
                .eq("quantity_type", value: "meal_patterns_score")
...
#Preview {
    MealPatternsScoreDetailView(
        viewModel: MealPatternsScoreViewModel(),
        color: .indigo
    )
}
