//
//  CaffeineScoreDetailView.swift
//  WellPath
//
//  Detail view showing caffeine score with Today/History/Baseline tabs.
//

import SwiftUI

struct CaffeineScoreDetailView: View {
    @ObservedObject var viewModel: CaffeineScoreViewModel
    let color: Color
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("Caffeine Score")
    ...
    private var noDataTodayView: some View {
        VStack(spacing: 16) {
            ...
            Text("No caffeine logged today")
                .font(.headline)
            Text("Log your caffeine intake to see today's score")
                .font(.subheadline)
                .foregroundColor(.secondary)
    ...
    private func loadScoreHistory() async {
        do {
            ...
            let results: [ScoreResult] = try await client
                ...
                .eq("quantity_type", value: "caffeine_score")
...
#Preview {
    CaffeineScoreDetailView(
        viewModel: CaffeineScoreViewModel(),
        color: .brown
    )
}
