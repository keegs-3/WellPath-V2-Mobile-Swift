//
//  CaffeineAmountCard.swift
//  WellPath
//
//  Reusable card for Caffeine Amount metric.
//

import SwiftUI

struct CaffeineAmountCard: View {
    let color: Color
    let pillar: String
    ...
    var body: some View {
        MetricCardView(
            title: "Caffeine Amount",
            color: color,
            metricId: "DISP_CAFFEINE_MG",
            pillar: pillar
        ) {
            CaffeineAmountMiniCard(viewModel: viewModel, color: color)
        } fullScreen: {
...
struct CaffeineAmountMiniCard: View {
...
#Preview {
    CaffeineAmountCard(color: .brown, pillar: "Healthful Nutrition")
        .padding()
}
