...
//  WellPath
//
//  Detail screen for Fruits with Timing and Type views
//

import SwiftUI

enum FruitsDetailTab: String, CaseIterable {
    case timing = "Timing"
    case type = "Type"
}

struct FruitsDetail: View {
    @State private var selectedTab: FruitsDetailTab = .timing

    let color = MetricsUIConfig.getPillarColor(for: "Healthful Nutrition")
    let screenIcon = MetricsUIConfig.getIcon(for: "Fruits")

    var body: some View {
        VStack(spacing: 0) {
            // Tab picker
            Picker("View", selection: $selectedTab) {
                ForEach(FruitsDetailTab.allCases, id: \.self) { tab in
        ...
        .navigationTitle("Fruits Details")
...
struct FruitsTimingView: View {
    let color: Color
    @StateObject private var educationViewModel = TabEducationViewModel(metricId: "DISP_FRUITS_MEAL_TIMING")
    ...
    var body: some View {
        VStack(spacing: 0) {
            if showAbout {
                ...
                NutrientTimingTimelineView(nutrientType: .fruits, color: color, showAbout: $showAbout)
            }
        }
        .task {
...
struct FruitsTypeView: View {
    let color: Color
    @StateObject private var educationViewModel = TabEducationViewModel(metricId: "DISP_FRUITS_TYPE")
    @State private var showAbout = false
    ...
    var body: some View {
        VStack(spacing: 0) {
            if showAbout {
                ...
                NutrientTypeView(nutrientType: .fruits, color: color, showAbout: $showAbout)
            }
        }
        .task {
            await educationViewModel.loadEducation()
