...
//  WellPath
//
//  Detail screen for Legumes with Timing and Type views
//

import SwiftUI

enum LegumesDetailTab: String, CaseIterable {
    case timing = "Timing"
    case type = "Type"
}

struct LegumesDetail: View {
    @State private var selectedTab: LegumesDetailTab = .timing

    let color = MetricsUIConfig.getPillarColor(for: "Healthful Nutrition")
    let screenIcon = MetricsUIConfig.getIcon(for: "Legumes")

    var body: some View {
        VStack(spacing: 0) {
            // Tab picker
            Picker("View", selection: $selectedTab) {
                ForEach(LegumesDetailTab.allCases, id: \.self) { tab in
        ...
        .navigationTitle("Legumes Details")
...
struct LegumesTimingView: View {
    let color: Color
    @StateObject private var educationViewModel = TabEducationViewModel(metricId: "DISP_LEGUMES_MEAL_TIMING")
    ...
    var body: some View {
        VStack(spacing: 0) {
            if showAbout {
                ...
                NutrientTimingTimelineView(nutrientType: .legumes, color: color, showAbout: $showAbout)
            }
        }
        .task {
...
struct LegumesTypeView: View {
    let color: Color
    @StateObject private var educationViewModel = TabEducationViewModel(metricId: "DISP_LEGUMES_TYPE")
    @State private var showAbout = false
    ...
    var body: some View {
        VStack(spacing: 0) {
            if showAbout {
                ...
                NutrientTypeView(nutrientType: .legumes, color: color, showAbout: $showAbout)
            }
        }
        .task {
            await educationViewModel.loadEducation()
