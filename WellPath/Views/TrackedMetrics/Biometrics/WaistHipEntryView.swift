    ...
    private func saveEntries() async {
        ...
        do {
            ...
            if let waist = Double(waistValue), waist > 0 {
                let waistSample = PatientSample.quantity(
                    patientId: userId,
                    quantityType: QuantityTypes.waistMeasurement,
                    value: waist,
            ...
            if let hip = Double(hipValue), hip > 0 {
                let hipSample = PatientSample.quantity(
                    patientId: userId,
                    quantityType: QuantityTypes.hipMeasurement,
                    value: hip,
                    unit: selectedUnit.databaseUnit,
                    timestamp: selectedDateTime,
                    source: .wellpathInput,
